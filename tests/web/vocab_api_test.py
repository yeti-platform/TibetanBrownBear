...
def test_add_value_to_vocab(authenticated_client):
    ...
    rv = authenticated_client.post('/api/settings/vocabs/malware-label-ov/',
...
@pytest.mark.usefixtures('clean_db', 'populate_settings')
def test_set_value_for_vocab(authenticated_client):
    """Tests that we can set all values for a vocab."""
    rv = authenticated_client.put('/api/settings/vocabs/malware-label-ov/',
                                  data=json.dumps({'values': ['trojan']}),
                                  content_type='application/json')
    response = json.loads(rv.data)
    assert sorted(response) == sorted(['trojan'])
...
def test_add_vocab_undefined_field(authenticated_client):
    ...
    authenticated_client.post('/api/settings/vocabs/Malware.newField/',
